...
X_train[cat_df.columns.tolist()] = X_train[cat_df.columns.tolist()].apply(LabelEncoder().fit_transform)
X_test[cat_df.columns.tolist()] = X_test[cat_df.columns.tolist()].apply(LabelEncoder().fit_transform)

y_train = y_train.replace(regex='Yes', value=1)
y_train = y_train.replace(regex='No', value=0)
y_test =  y_test.replace(regex='Yes', value=1)
y_test =  y_test.replace(regex='No', value=0)
